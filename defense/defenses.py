...
def ransacmodel(x, y, model, lam, counts):
    allmodels = {'lasso': lm.Lasso(alpha=lam, max_iter=10000), 'ridge': lm.Ridge(alpha=lam),
                 'enet': lm.ElasticNet(alpha=lam, max_iter=10000), 'linreg': lm.Ridge(alpha=0.00001)}
    scores = []
    ...
    for trainsize in trainsizes:
        ...
        for i in range(x.shape[0]):
            ...
            for j in range(RONI_TRIALS):
                ...
                include, _ = learnmodel(np.append(trainx, curx.reshape((1, -1)), axis=0),
